...
const TeamMember = ({ teamMember, setActiveTeamMember, teamMemberIndex }) => (
  <li
    key={teamMember.title.rendered.toLowerCase().replace(/ /g, `-`)}
    css={[
      css`
        &:hover {
  ...
  >
    ...
    >
      ...
      >
        {(teamMember.team_featured_media && (
          <figure
            css={[
              tw`w-full md:h-full md:absolute md:top-0 md:right-0 md:bottom-0 md:left-0`,
            ]}
          >
            <img
              src={teamMember.team_featured_media}
              css={[
                css`
                  @media (max-width: 776px) {
                    height: 185px;
        ...
        <div
          css={[
            css`
              ...
              background: ${theme`colors`[teamMember.acf.color || "red"]};
            `,
        ...
        <article
          css={[
            css`
              ...
              background: ${teamMember.acf.color || "#e5e7eb"};
              color: ${teamMember.acf.text_color || "black"};
            `,
            tw`w-full h-full relative z-10 flex flex-col items-center justify-center px-8`,
          ]}
        >
          <T.Heading font="b1">{teamMember.title.rendered}</T.Heading>

          <T.Body font="2" styles={[tw`mt-2`]}>
            {teamMember.acf.sub_title}
          </T.Body>
        </article>
      </button>
