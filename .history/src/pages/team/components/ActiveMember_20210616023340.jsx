...
const ActiveMember = ({
  ...
  <Grid node="section">
    ...
    >
      ...
      {activeTeamMember?.team_featured_media && (
        <div
          css={[
            css`
              padding-bottom: 70%;
        ...
        >
          <figure tw="w-full h-full absolute top-0 right-0 bottom-0 left-0 flex items-center justify-center">
            <img
              src={activeTeamMember.team_featured_media}
              css={[css``, tw`w-full h-full relative block object-cover`]}
              alt={activeTeamMember.title.rendered}
            />
          </figure>
        </div>
      )}
    ...
    <header css={[tw`col-span-12 md:col-span-3 md:col-start-3 mt-4 md:mt-0`]}>
      <T.Heading font="3" level="2">
        {activeTeamMember?.title.rendered || ``}
      </T.Heading>

      <T.Heading font="b1" styles={[tw`mt-2 mb-8 md:mb-0`]}>
        {activeTeamMember?.acf.sub_title || ``}
      </T.Heading>
    </header>
    ...
    <div
      css={[
        css`
          ...
          font-weight: 300;
        `,
        tw`col-span-12 md:col-span-5 mb-24 md:mb-64 text-base whitespace-pre-wrap`,
      ]}
      dangerouslySetInnerHTML={{
        __html: activeTeamMember?.content.rendered || ``,
      }}
    />
  </Grid>
);
