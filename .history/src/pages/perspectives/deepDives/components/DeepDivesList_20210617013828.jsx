...
const DeepDivesList = ({
  ...
  return (
    <ul css={[tw`relative`]}>
      {deepDivesItems[0].map((deepDive, deepDiveIndex) => {
        console.log("deepDive", deepDive);
        const { title, tagline } = deepDive;

        const key = `deep-dive-${deepDiveIndex}`;
        const active = expandedDeepDive === deepDiveIndex;
        console.log("deepDive title", title);
        console.log("deepDive active", active);
            ...
            <Grid>
              ...
              >
                <button
                  type="button"
                  onClick={() =>
                    setExpandedDeepDive(active ? null : deepDiveIndex)
                  }
                  css={[tw`relative block pt-10 md:pr-12 pb-10 text-left`]}
                >
                  <header>
                    <T.Heading font="3">{title.rendered}</T.Heading>
                  </header>
                </button>
                {active && (
                  <ul css={[tw`w-full relative mt-8 md:mt-20`]}>
                    {deepDivesItems[0].map((article, articleIndex) => (
                      <li
                        css={[
                          css`
                            ${A.Keyframes(
                      ...
                      >
                        <Go to={`/perspectives/${article.slug}`}>
